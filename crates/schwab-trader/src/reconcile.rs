...
pub async fn reconcile_tick(
    ...
    for (symbol, live_pos) in &live_by_symbol {
        ...
        let (profit_limit, stop_px, _) = exit_prices(live_pos.average_price, rules, None, None);
    ...
    for mut ub in unbracketed {
        ...
        let (profit_limit, stop_px, stop_limit_px) = exit_prices(ub.entry_price, rules, None, None);
...
async fn adopt_filled_buy(
    ...
    let pos_id = position_id(symbol, &rules.schedule.timezone);
    let (profit_limit, stop_px, stop_limit_px) = exit_prices(entry_price, rules, None, None);
