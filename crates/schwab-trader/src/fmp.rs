...
pub fn write_discovered_pool(path: &Path, symbols: &[String], label: &str) -> Result<()> {
    ...
    body.push_str("# Run `watchlist build --write` next so Schwab playbook filters apply.\n");
...
pub async fn apply_fmp_discover_if_due(
    ...
    // Drop previous FMP symbols + blocked / already-open clutter.
    let prev: std::collections::BTreeSet<String> = state
        .fmp_dynamic_symbols
        .iter()
        .map(|s| s.to_uppercase())
        .collect();
    let open: std::collections::BTreeSet<String> = state
        .open_positions
        .values()
    ...
    // Reserve room for this refresh's FMP adds.
    while state.dynamic_watchlist.len() + max_add > max_dynamic
        && !state.dynamic_watchlist.is_empty()
    {
        // Evict oldest non-open dynamic name.
    ...
    for row in &discovered.candidates {
        ...
        state.dynamic_watchlist.push(sym.clone());
        added.push(sym);
    }
